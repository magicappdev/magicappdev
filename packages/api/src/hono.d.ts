...
declare module "hono" {
  export interface Context<E = {}, P extends string = string, I = {}> {
    ...
    req: HonoRequest<P, I>;
    get<K extends keyof (E extends { Variables: infer V } ? V : Record<string, unknown>)>(
      key: K
    ): (E extends { Variables: infer V } ? V : Record<string, unknown>)[K];
    set<K extends keyof (E extends { Variables: infer V } ? V : Record<string, unknown>)>(
      key: K,
      value: (E extends { Variables: infer V } ? V : Record<string, unknown>)[K]
    ): void;
  ...
  export type Handler<E = {}, P extends string = string, I = {}, R = Response> = (
    c: Context<E, P, I>
  ) => R | Promise<R>;
  ...
  export class Hono<E = {}, S = {}, B extends string = "/"> {
    ...
    use<P extends string = "*">(path: P, ...handlers: MiddlewareHandler<E>[]): this;
    ...
    route<SubPath extends string, SubEnv>(path: SubPath, app: Hono<SubEnv>): this;
    ...
    fetch(request: Request, env?: E extends { Bindings: infer B } ? B : unknown, ctx?: ExecutionContext): Promise<Response>;
  }
}
...
declare module "hono/jwt" {
  ...
  export function sign(payload: Record<string, unknown>, secret: string): Promise<string>;
  export function verify<T = Record<string, unknown>>(token: string, secret: string): Promise<T>;
  export function decode<T = Record<string, unknown>>(token: string): { header: unknown; payload: T };
}
