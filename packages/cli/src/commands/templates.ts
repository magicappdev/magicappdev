...
async function listTemplates(options: {
  ...
  try {
    ...
    for (const t of filtered) {
      ...
      console.log(
        `  ${"".padEnd(20)} Category: ${t.category} | Frameworks: ${t.frameworks.join(", ")} | v${t.version}`,
      );
      console.log();
    }
  } catch (err) {
    error(`Failed to list templates: ${(err as Error).message}`);
    process.exit(1);
  }
}
...
async function showTemplate(
  ...
  try {
    ...
    error(`Failed to fetch template "${slug}": ${(err as Error).message}`);
    process.exit(1);
  }
}

/** Download and cache a template */
async function downloadTemplate(slug: string): Promise<void> {
  ...
  try {
    ...
    success(
      `Template "${template.name}" (v${template.version}) downloaded to ${templateDir}`,
    );
    info(`Use: magicappdev init --template ${slug}`);
  } catch (err) {
    error(`Failed to download template "${slug}": ${(err as Error).message}`);
    process.exit(1);
  }
}
...
async function syncTemplates(): Promise<void> {
  ...
  try {
    ...
    error(`Failed to sync templates: ${(err as Error).message}`);
    process.exit(1);
  }
}
...
async function deleteTemplate(slug: string): Promise<void> {
  ...
  if (!existsSync(templateDir)) {
    error(
      `Template "${slug}" is not cached. Run: magicappdev templates download ${slug}`,
    );
    return;
  }
...
/** Show cache status */
async function cacheStatus(): Promise<void> {
  const cacheDir = getTemplateCacheDir();
...
`,
  ...
  .option(
    "-c, --category <name>",
    "Filter by category (app, component, screen)",
  )
  .option(
    "-f, --framework <name>",
    "Filter by framework (next, react, react-native)",
  )
