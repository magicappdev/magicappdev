 ...
 */
import { buttonComponentTemplate, contextTemplate, generateComponent, generateFromTemplate, generateScreen, hookTemplate, pageTemplate, screenTemplate, serviceTemplate, } from "@magicappdev/templates";
import { Command } from "commander";
import { promptText } from "../../lib/prompts.js";
import { withSpinner } from "../../lib/spinner.js";
import { error, header, info, keyValue, newline, success, } from "../../lib/ui.js";
...
`)
    ...
    try {
        ...
        newline();
        const result = await withSpinner(`Creating ${componentName}...`, () => generateComponent(componentName, buttonComponentTemplate, outputDir, {
    ...
    try {
        ...
        keyValue("Path", outputDir);
        newline();
        const result = await withSpinner(`Creating ${screenName}...`, () => generateScreen(screenName, screenTemplate, outputDir, {
            typescript: options.typescript ?? true,
    ...
    try {
        ...
        keyValue("Path", outputDir);
        newline();
        const result = await withSpinner(`Creating ${pageName} page...`, () => generateFromTemplate(pageTemplate, {
            outputDir,
    ...
    try {
        ...
        keyValue("Path", outputDir);
        newline();
        const result = await withSpinner(`Creating use${displayName}...`, () => generateFromTemplate(hookTemplate, {
            outputDir,
    ...
    .addCommand(new Command("service")
    .alias("sv")
    ...
    try {
        ...
        newline();
        const result = await withSpinner(`Creating ${resourceName}Service...`, () => generateFromTemplate(serviceTemplate, {
            outputDir,
            variables: {
                name: resourceName,
                typescript: options.typescript ?? true,
    ...
    try {
        ...
        keyValue("Path", outputDir);
        newline();
        const result = await withSpinner(`Creating ${contextName}Context...`, () => generateFromTemplate(contextTemplate, {
            outputDir,
