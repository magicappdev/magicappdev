...
import { Loader2, RefreshCw, Globe, ExternalLink, AlertCircle, Code2, Play } from "lucide-react";
...
export function LivePreview({ projectId, files }: LivePreviewProps) {
  ...
  const injectConsoleInterceptor = () => {
    ...
    try {
      iframe.contentWindow.postMessage({ type: "MAGICAPPDEV_CONSOLE_INIT" }, "*");
    } catch (err) {
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
        ...
        {previewStatus === "loading" && (
          <div className="absolute inset-0 flex items-center justify-center bg-background/80 backdrop-blur-sm">
            <div className="text-center">
              ...
              <Typography variant="body" className="text-sm">Loading preview...</Typography>
            </div>
          </div>
        )}
        ...
        <div className="h-48 border-t border-outline/10 bg-surface-variant flex flex-col">
          <div className="flex items-center justify-between px-3 py-2 border-b border-outline/10">
            ...
            <div className="flex items-center gap-2">
              <Button variant="text" size="sm" className="text-xs" onClick={() => setConsoleLogs([])}>
                Clear
              </Button>
              <Button variant="text" size="sm" className="h-6 w-6 p-0" onClick={() => setShowConsole(false)}>
                ×
              </Button>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-2 font-mono text-xs space-y-1">
            {consoleLogs.length === 0 ? (
              <div className="text-foreground/30 text-center py-4">No console output</div>
            ) : (
      ...
      <div className="border-t border-outline/10 bg-surface p-2 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <Button variant="text" size="sm" onClick={() => setShowConsole(!showConsole)}>
        ...
        <div className="flex items-center gap-1 text-xs text-foreground/40">
          <div className={`w-2 h-2 rounded-full ${
            ...
            previewStatus === "loading" ? "bg-yellow-500 animate-pulse" :
            "bg-red-500"
          }`} />
