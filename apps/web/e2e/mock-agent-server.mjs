...
wss.on("connection", (ws, req) => {
  ...
  ws.on("message", raw => {
    ...
    const streamInterval = setInterval(() => {
      if (i >= words.length) {
        ...
        console.log(`[mock-agent] response complete for ${clientId.slice(0, 8)}`);
        return;
      }
  ...
  ws.on("error", err => {
    console.error(`[mock-agent] error for ${clientId.slice(0, 8)}:`, err.message);
  });
});

server.listen(PORT, () => {
  console.log(`[mock-agent] HTTP listening on http://localhost:${PORT}/health`);
  console.log(`[mock-agent] WS   listening on ws://localhost:${PORT}${WS_PATH}`);
});