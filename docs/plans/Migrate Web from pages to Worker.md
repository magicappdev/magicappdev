...
description: A guide for migrating from Cloudflare Pages to Cloudflare Workers.
  ...
  Workers and Pages.
lastUpdated: 2026-01-05T14:52:46.000Z
...
## + Change name of the worker so its fitting nicely into the worker sub domain (<workername>.magicappdev.workers.dev) ?? build? build.magicappdev.workers.dev ?? or web? web.magicappdev.workers.dev ?? create? create.magicappdev.workers.dev ?? or something else?

- Dont forget to change the domain name in the wrangler.toml file to the new name. keep package name.
- Dont forget implementation + bindings to other workers.: api, db, storage, ai, etc.
